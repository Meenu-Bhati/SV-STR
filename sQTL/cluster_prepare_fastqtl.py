...
if __name__=='__main__':
    ...
    bed_df['chr_ix'] = bed_df['#Chr'].str.replace('chr','').str.replace('X','29').astype(np.int32)
    for c in ['start', 'end']:
    ...
    bed_file = os.path.join(args.output_dir, f'{args.prefix}.perind.counts.filtered.qqnorm.bed')
    bed_df.to_csv(bed_file, sep='\t')       
    ...
    gene_bed_df_file = os.path.join(args.output_dir, f'{args.prefix}.leafcutter.bed')
    gene_bed_df.to_csv(gene_bed_df_file, sep='\t')       
    ...
    os.remove(os.path.join(args.output_dir, f'{args.prefix}.perind.counts.filtered.qqnorm.bed'))
